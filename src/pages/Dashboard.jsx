...
const Dashboard = () => {
  ...
  const fetchData = async () => {
    try {
      ...
      if (isSuperAdmin) {
        ...
        endpoints.push(API.get("/admin/dashboard/superadmin-income-stats"));
      }
      ...
      if (isSuperAdmin) {
        ...
        setIncomeStats(results[5].data.stats);
      }
    } catch (error) {
  ...
  const StatCard = ({ title, value, icon, color, onClicked }) => (
    <Card 
      onClick={onClicked}
      sx={{ 
        borderRadius: 4, 
  ...
  return (
    <Box sx={{ p: { xs: 2, md: 4 }, bgcolor: "#f8f9fa", minHeight: "100vh" }}>
      ...
      {/* Superadmin Income Cards */}
      {isSuperAdmin && incomeStats && (
        <Grid container spacing={3} sx={{ mb: 4 }}>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              title="Today's Income"
              value={`₹${incomeStats.todayIncome.toLocaleString()}`}
          ...
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              title="Yearly Income"
              value={`₹${incomeStats.yearlyIncome.toLocaleString()}`}
              icon={<TrendingUpOutlined />}
              color="#FD9644"
            />
          </Grid>
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              title="Total (Since 1/1/2026)"
              value={`₹${incomeStats.totalIncomeSince2026.toLocaleString()}`}
              icon={<TrendingUpOutlined />}
              color="#1a237e"
            />
          </Grid>
        ...
        {(isSuperAdmin || isAdmin) && (
          <Grid item xs={12} sm={6} md={3}>
            <StatCard
              ...
              color="#00B894"
              onClicked={() => navigate("/supervisors")}
            />
          </Grid>
        )}
        <Grid item xs={12} sm={6} md={3}>
      ...
      <Grid container spacing={3} sx={{ mb: 4 }}>
        ...
        <Grid item xs={12} sm={6}>
          <StatCard
            title="Available Quantity in Inventory"
